...
const createPost = asyncHandler(async (req, res) => {
  ...
  const [result] = await pool.query(
    ...
    [slug, b.title, b.excerpt || null, b.content || '', b.featured_image || null, b.category || null,
      ...
      b.keywords || null, b.is_published ? 1 : 0, b.is_published ? 1 : 0]
  );
...
const updatePost = asyncHandler(async (req, res) => {
  const fields = ['title', 'excerpt', 'content', 'featured_image', 'category', 'author_name',
    'meta_title', 'meta_description', 'keywords', 'is_published'].filter((f) => f in req.body);
...
const adminList = asyncHandler(async (req, res) => {
  const [rows] = await pool.query('SELECT id, slug, title, category, is_published, published_at, created_at FROM blog_posts ORDER BY created_at DESC');
